...
def parse_args():
    ...
    p.add_argument("--dataset-path", required=True,
                   help="Local aligned dataset dir (e.g., data/4b00df29-3769-43be-bb40-128b1cba6d35_aligned)")
...
def main():
    ...
    from utils.eval_utils import co_smoothing_eval
    ...
    # 加载配置
    config = config_from_kwargs({"model": f"include:{args.model_config}"})
    config = update_config(args.model_config, config)
    config = update_config(args.trainer_config, config)

    # 加载本地数据集
    logger.info("Loading local dataset...")
    local_dataset = load_from_disk(args.dataset_path)
    test_dataset = local_dataset["test"]
    logger.info(f"Loaded local test set with {len(test_dataset)} trials")

    # 获取神经元数量
    n_neurons = len(test_dataset['cluster_regions'][0])
    ...
    test_dataloader = make_loader(
        ...
        batch_size=len(test_dataset),  # 一次加载所有数据
        pad_to_right=True,
        pad_value=-1.,
    ...
    # 从 meta_data 获取 num_neurons
    meta_data = {"num_neurons": [n_neurons]}

    model_class = {"NDT1": NDT1}.get(config.model.model_class)
    ...
    logger.info(f"(eval) masking ratio: {model.encoder.masker.ratio}")
